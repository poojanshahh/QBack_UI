// taken from https://www.npmjs.com/package/react-table-sticky v1.1.3
...
export const checkErrors = (columns: Column[]) => {
  ...
  const bugWithUnderColumnsSticky = columns.find(
    parentCol => !parentCol.sticky && parentCol.columns && parentCol.columns.find(col => col.sticky)
  );
...
export const getStickyValue = (column: Column): null | 'left' | 'right' => {
...
export const columnIsLastLeftSticky = (columnId: Column['id'], columns: any): boolean => {
...
export const columnIsFirstRightSticky = (columnId: Column['id'], columns: any): boolean => {
  const index = columns.findIndex(({ id }: any) => id === columnId);
  const column = columns[index];
...
export const getMarginRight = (columnId: Column['id'], columns: any) => {
...
const findHeadersSameLevel = (header: any, headers: any) => {
